/**
 * @file switch.c
 * @brief A packet switch implementation that captures packets from two
 * interfaces (ethsec and ethinsec) and forwards them to the other interface.
 *
 * This program creates raw sockets for two network interfaces (ethsec and
 * ethinsec), captures packets from each interface, and publishes them to NATS
 * topics. It uses pthreads to handle packet capturing and forwarding
 * concurrently. The program integrates with NATS for message publishing and
 * subscribing:
 * - Captured packets are published to NATS subjects ("inpktfromsec" and
 * "inpktfrominsec").
 * - Subscribes to NATS subjects ("outpktsec" and "outpktinsec") to receive
 * packets from packet processors and forwards them to the appropriate
 * interface.
 *
 * @dependencies
 * - pthread
 ...
 * 8. Subscribes to NATS subjects to receive packets and forwards them to the
 ...
 * @functions
 * - int main()
 *   - Entry point of the program. Initializes raw sockets, binds them to
 * interfaces, and creates threads for packet capturing.
 *
 * - void *capture_packets(void *arg)
 *   - Captures packets from the specified interface and forwards them to the
 * other interface.
 *   - @param arg Pointer to the interface name (ethsec or ethinsec).
 *   - @return NULL
 *
 * - void handle_packet_from_interface(unsigned char *buffer, int size, char
 * *in_iface)
 ...
 *   - Configures the switch by reading environment variables and querying MAC
 * addresses.
 ...
 *
 * - void query_mac_address(char *interface, char *host_ip, unsigned char
 * *mac_address)
...
void *capture_packets(void *arg);
void handle_packet_from_nats(natsConnection *nc, natsSubscription *sub,
                             natsMsg *msg, void *closure);
void handle_packet_from_interface(unsigned char *buffer, int size,
                                  char *in_iface);
bool configure_switch();
bool configureRawSockets();
bool configure_nats();
char *get_interface_for_subnet(char *subnet);
void query_mac_address(char *interface, char *host_ip,
                       unsigned char *mac_address);
void query_mac_address_with_arp(char *interface, char *host_ip,
                                unsigned char *mac_address);
void query_mac_address_with_arp_query(char *interface, char *host_ip,
                                      unsigned char *mac_address);
...
int main() {
  ...
  // READ ENVIRONMENT VARIABLES and CONFIGURE IP ADDRESS, MAC ADDRESS, INTERFACE
  // NAMES
  bool configured = configure_switch();
  if (!configured) {
    fprintf(stderr, "Switch configuration failed; check your docker-compose "
                    "settings, define env variables correctly.\n");
    exit(1);
  } else {
  ...
  if (!configured) {
    fprintf(stderr,
            "Raw socket configuration failed; check your docker-compose "
            "settings, define env variables correctly.\n");
    exit(1);
  } else {
  ...
  if (!configured) {
    fprintf(stderr, "NATS configuration failed; check your docker-compose "
                    "settings, define env variables correctly.\n");
    exit(1);
  } else {
...
void *capture_packets(void *arg) {
  ...
  while (1) {
    int data_size = recvfrom(sock_raw, buffer, BUF_SIZE, 0, &saddr,
                             (socklen_t *)&saddr_len);
    if (data_size < 0) {
...
bool configure_switch() {
  ...
  query_mac_address_with_arp_query(ethsec, secure_net_host_ip,
                                   mac_secure_net_host);
  query_mac_address_with_arp_query(ethinsec, insecure_net_host_ip,
                                   mac_insecure_net_host);

  printf("MAC address of (%s): %02x:%02x:%02x:%02x:%02x:%02x\n",
         secure_net_host_ip, mac_secure_net_host[0], mac_secure_net_host[1],
         mac_secure_net_host[2], mac_secure_net_host[3], mac_secure_net_host[4],
         mac_secure_net_host[5]);

  printf("MAC address of (%s): %02x:%02x:%02x:%02x:%02x:%02x\n",
         insecure_net_host_ip, mac_insecure_net_host[0],
         mac_insecure_net_host[1], mac_insecure_net_host[2],
         mac_insecure_net_host[3], mac_insecure_net_host[4],
         mac_insecure_net_host[5]);
...
bool configureRawSockets() {
  ...
  if (bind(sock_raw_ethsec, (struct sockaddr *)&sll_ethsec,
           sizeof(sll_ethsec)) < 0) {
  ...
  printf("MAC address of ethsec: %02x:%02x:%02x:%02x:%02x:%02x\n",
         mac_ethsec[0], mac_ethsec[1], mac_ethsec[2], mac_ethsec[3],
         mac_ethsec[4], mac_ethsec[5]);
  ...
  if (bind(sock_raw_ethinsec, (struct sockaddr *)&sll_ethinsec,
           sizeof(sll_ethinsec)) < 0) {
  ...
  printf("MAC address of ethinsec: %02x:%02x:%02x:%02x:%02x:%02x\n",
         mac_ethinsec[0], mac_ethinsec[1], mac_ethinsec[2], mac_ethinsec[3],
         mac_ethinsec[4], mac_ethinsec[5]);
...
bool configure_nats() {
  ...
  s = natsConnection_Subscribe(&sub_outpktsec, conn, "outpktsec",
                               handle_packet_from_nats, NULL);
  if (s != NATS_OK) {
    fprintf(stderr, "Error subscribing to outpktsec: %s\n",
            natsStatus_GetText(s));
    configured = false;
  }
  s = natsConnection_Subscribe(&sub_outpktinsec, conn, "outpktinsec",
                               handle_packet_from_nats, NULL);
  if (s != NATS_OK) {
...
char *get_interface_for_subnet(char *subnet) {
  ...
  // Shell command to list interfaces and fetch the interface name that has the
  // subnet Run the shell command to find the interface name
...
void query_mac_address_with_arp_query(char *interface, char *host_ip,
  ...
  printf("Queried MAC address for %s on interface %s: "
         "%02x:%02x:%02x:%02x:%02x:%02x\n",
         host_ip, interface, mac_address[0], mac_address[1], mac_address[2],
...
void query_mac_address_with_arp_cache(char *interface, char *host_ip,
                                      unsigned char *mac_address) {
  ...
  while (fgets(line, sizeof(line), arp_cache)) {
    char ip[INET_ADDRSTRLEN], hw_type[8], flags[8], mac[18], mask[8],
        dev[IFNAMSIZ];
    if (sscanf(line, "%s %s %s %s %s %s", ip, hw_type, flags, mac, mask, dev) ==
        6) {
      if (strcmp(ip, host_ip) == 0 && strcmp(dev, interface) == 0) {
        sscanf(mac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac_address[0],
  ...
  if (!found) {
    fprintf(stderr,
            "MAC address for IP %s on interface %s not found in ARP cache\n",
            host_ip, interface);
    exit(1);
  }
}
...
// For example, 02:42:0a:00:00:15 with 02:42 and then the 10.0.0.21 in hex
// byte-by-byte.
void query_mac_address(char *interface, char *host_ip,
                       unsigned char *mac_address) {
  mac_address[0] = 0x02;
...
void print_packet(unsigned char *buffer, int size, char *iface,
  ...
  if (ntohs(eth->h_proto) == ETH_P_IP) {
    ...
    if (iph->protocol == IPPROTO_TCP) {
      ...
      struct icmphdr *icmph =
          (struct icmphdr *)(buffer + iph->ihl * 4 + sizeof(struct ethhdr));
      printf("ICMP Header:\n");
  ...
  printf("Interface: %s (%s), %s SMAC: %02x:%02x:%02x:%02x:%02x:%02x, DMAC: "
         "%02x:%02x:%02x:%02x:%02x:%02x, Protocol: %s\n",
